...
try
{
    ...
    using (var vault = new CredentialVault(vaultRoot))
    {
        ...
        await vault.SaveAsync("account_a", credential);
        await vault.SaveAsync("account_b", credential with { AccessToken = "another-synthetic-access" });
        ...
        string master = Path.Combine(vaultRoot, "master.key.dpapi"); byte[] originalMaster = File.ReadAllBytes(master);
        File.Delete(master);
        ...
        Check(File.ReadAllBytes(pathA).SequenceEqual(originalA), "wrong-key save preserves old ciphertext");
        File.WriteAllBytes(master, originalMaster);
    ...
    var indexer = new UsageIndexer(database);
    ...
    await File.WriteAllTextAsync(file, originalText, new UTF8Encoding(false));
    await indexer.ScanAsync(Provider.Codex, [sessions], true);
    ...
    foreach (string storage in Directory.EnumerateFiles(database.Root, "quotalens.sqlite*"))
        Check(!Encoding.UTF8.GetString(File.ReadAllBytes(storage)).Contains("SYNTHETIC-CONVERSATION-DO-NOT-INDEX"), "analytical storage excludes conversation bodies: " + Path.GetExtension(storage));
    ...
    await indexer.ScanFileAsync(Provider.Codex, copy, true);
    var recovery = new RecoveryService(database, indexer);
    var copySource = (await database.SourcesAsync(Provider.Codex)).Single(x => x.Path == copy);
