...
/// <summary>Query identities are independent of tool logins. Provider gates serialize refresh-token rotation,
/// imports, queries and removal; independent providers can run concurrently.</summary>
public sealed class AccountService(LocalDatabase database, ICredentialVault vault, ProviderClient client, Func<AppSettings> settings)
{
    ...
    public AccountView? View(string? key) => key is null ? null : Views.FirstOrDefault(x => x.Account.Key == key);
    ...
    {
        foreach (var account in await database.AccountsAsync(ct).ConfigureAwait(false))
        {
            ...
            try
            {
                ...
                if (await database.GetMetadataAsync<CodexExtendedSnapshot>("extended_" + account.Key, ct).ConfigureAwait(false) is { } extra && extra.Quota.AccountKey == account.Key) extended[account.Key] = extra;
                if (await database.GetMetadataAsync<SubscriptionInfo>("subscription_" + account.Key, ct).ConfigureAwait(false) is { } subscription && subscription.AccountKey == account.Key) subscriptions[account.Key] = subscription;
        ...
        // Actual tool identities are revalidated only through explicit local-import consent, never from a viewing selection.
    ...
    private void SetFailure(string key, Exception error)
    {
        ...
        statuses[key] = new(snapshots.GetValueOrDefault(key)?.ObservedAt, safe.Kind, safe.Message, safe.RetryAt); Notify();
    }
    private async Task<Credential> LoadAsync(Account account, CancellationToken ct)
    {
        ...
        if (value.Provider != account.Provider) throw new QuotaException(FailureKind.Identity, "Credential provider does not match this account.");
        return value;
    }
    private async Task SaveRotatedAsync(Account account, Credential credential, CancellationToken ct)
    {
        ...
        if (account.Provider == Provider.Codex && JsonTools.JwtClaims(credential.AccessToken).At("https://api.openai.com/auth", "chatgpt_account_id").Text() != account.ProviderId)
            throw new QuotaException(FailureKind.Identity, "The returned Codex credential belongs to another account.");
        // Keep rotated credentials before disk I/O so a failed save cannot cause a second rotation with the old token.
    ...
    private async Task<Credential> FreshAsync(Account account, Credential credential, CancellationToken ct, bool force = false)
    {
        ...
        bool stillValid = credential.AccessToken.Length > 0 && (credential.EffectiveExpiry is null || credential.EffectiveExpiry > DateTimeOffset.UtcNow);
        if (credential.RefreshToken is null)
        {
            if (!force && stillValid) return credential;
            throw new QuotaException(FailureKind.Expired, "This account requires authorization again; no refresh token is available.");
        }
        if (credential.Provider == Provider.Antigravity && client.GoogleConfiguration is null && !force && stillValid) return credential;
        var updated = await client.RefreshAsync(credential, ct).ConfigureAwait(false);
        await SaveRotatedAsync(account, updated, ct).ConfigureAwait(false); return updated;
    }
    private async Task<(QueryResult Result, Credential Credential)> QueryVerifiedAsync(Account expected, Credential credential, CancellationToken ct)
    {
        ...
        {
            credential = await FreshAsync(expected, credential, ct, true).ConfigureAwait(false);
    ...
    {
        RequireEnabled(input.Provider); await gates[input.Provider].WaitAsync(ct).ConfigureAwait(false);
        try
        {
            ...
            bool usableGoogle = credential.Provider == Provider.Antigravity && client.GoogleConfiguration is null && credential.AccessToken.Length > 0 &&
                (credential.EffectiveExpiry is null || credential.EffectiveExpiry > DateTimeOffset.UtcNow);
            if (credential.NeedsRefresh(DateTimeOffset.UtcNow) && credential.RefreshToken is not null && !usableGoogle)
            {
                credential = await client.RefreshAsync(credential, ct).ConfigureAwait(false); rotatedImports[fingerprint] = credential;
            }
            QueryResult result;
            try { result = await client.QueryAsync(credential, ct).ConfigureAwait(false); }
            catch (QuotaException error) when (error.Kind == FailureKind.Expired && credential.RefreshToken is not null)
            ...
            bool preserveIndependent = false;
            if (source == "local" && previous?.Source == "independent" && statuses.GetValueOrDefault(previous.Key)?.Failure is not (FailureKind.Revoked or FailureKind.Expired or FailureKind.NotConnected))
            {
                var saved = await LoadAsync(previous, ct).ConfigureAwait(false);
                preserveIndependent = saved.AccessToken.Length > 0 && (saved.EffectiveExpiry is null || saved.EffectiveExpiry > DateTimeOffset.UtcNow || saved.RefreshToken is not null);
            }
    ...
    public async Task<ImportResult> ImportLocalAsync(Provider provider, bool explicitImport, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (!explicitImport && localIdentities.TryGetValue(provider, out var saved) && saved.Fingerprint == local.Fingerprint && accounts.TryGetValue(saved.AccountKey, out var account)) return new(account, false);
            return await ImportAsync(local.Credential, "local", explicitImport, local.Fingerprint, ct).ConfigureAwait(false);
        }
        catch { localIdentities.TryRemove(provider, out _); Notify(); throw; }
    }
    public async Task RefreshAsync(string key, bool force, CancellationToken ct)
    {
        ...
        var provider = account.Provider; RequireEnabled(provider); await gates[provider].WaitAsync(ct).ConfigureAwait(false);
        try
        {
            if (!accounts.TryGetValue(key, out account)) return;
            if (statuses.GetValueOrDefault(key)?.RetryAt > DateTimeOffset.UtcNow) return;
            if (!force && snapshots.GetValueOrDefault(key)?.ObservedAt > DateTimeOffset.UtcNow.AddSeconds(-45)) return;
            var (result, fresh) = await QueryVerifiedAsync(account, await LoadAsync(account, ct).ConfigureAwait(false), ct).ConfigureAwait(false);
            var quota = result.Snapshot; string? warning = null;
            if (provider == Provider.Codex && settings().CollectCodexCloudUsage)
            {
                try
                {
            ...
            await database.SaveQuotaAsync(quota, subscriptions.GetValueOrDefault(key)?.Plan, ct).ConfigureAwait(false); snapshots[key] = quota;
        ...
        finally { gates[provider].Release(); }
    }
    public async Task RenewExpiringAsync(CancellationToken ct)
    {
        foreach (var account in accounts.Values.Where(x => IsEnabled(x.Provider)))
        {
            ...
            try
            {
                ...
                if (credential.RefreshToken is not null && credential.NeedsRefresh(DateTimeOffset.UtcNow)) await FreshAsync(account, credential, ct).ConfigureAwait(false);
                else if (liveCredentials.ContainsKey(account.Key)) await vault.SaveAsync(account.Key, credential, ct).ConfigureAwait(false);
            }
            catch (Exception error) when (error is not OperationCanceledException) { SetFailure(account.Key, error); }
    ...
    public async Task RenameAsync(string key, string alias, CancellationToken ct)
    {
        ...
        var provider = account.Provider; await gates[provider].WaitAsync(ct).ConfigureAwait(false);
        try
        {
            if (!accounts.TryGetValue(key, out account)) return;
    ...
    private async Task<T> RunCodexLockedAsync<T>(Account account, Credential credential, Func<CodexRpc, CancellationToken, Task<T>> action, CancellationToken ct)
    {
        ...
        finally
        {
            ...
            // A persistence failure propagates; keep the private runtime for startup recovery.
    ...
    public async Task<Credential> AuthorizeCodexAsync(Func<Uri, Task> openBrowser, CancellationToken ct)
    {
        ...
        var home = await PrivateCodexHome.CreateAsync(database.Root, null, null, ct).ConfigureAwait(false); CodexRpc? rpc = null;
        try
        {
            rpc = await CodexRpc.StartAsync(executable, home.Path, client.ApplicationVersion, ct).ConfigureAwait(false);
            await rpc.AuthorizeAsync(openBrowser, ct).ConfigureAwait(false);
            await rpc.DisposeAsync().ConfigureAwait(false); rpc = null;
            return await home.ReadCredentialAsync(ct).ConfigureAwait(false) ?? throw new QuotaException(FailureKind.NotConnected, "Codex did not save supported file-based authorization.");
        }
        finally { if (rpc is not null) await rpc.DisposeAsync().ConfigureAwait(false); home.Delete(); }
    }
    public async Task<CodexExtendedSnapshot> LoadCreditsAsync(string key, CancellationToken ct)
    {
        ...
        RequireEnabled(account.Provider); if (account.Provider != Provider.Codex) throw new ArgumentException("Codex account required.");
    ...
    {
        var account = accounts.GetValueOrDefault(key) ?? throw new QuotaException(FailureKind.NotConnected, "Choose a verified account.");
        RequireEnabled(Provider.Codex); if (account.Provider != Provider.Codex) throw new ArgumentException("Codex account required.");
        await gates[Provider.Codex].WaitAsync(ct).ConfigureAwait(false);
        try
        {
            var credential = await FreshAsync(account, await LoadAsync(account, ct).ConfigureAwait(false), ct).ConfigureAwait(false);
            return await RunCodexLockedAsync(account, credential, async (rpc, token) => {
                ...
                try
                {
                    ...
                    await database.SaveRedemptionAsync(request with { State = outcome }, CancellationToken.None).ConfigureAwait(false); return outcome;
                }
                catch
                {
                    await database.SaveRedemptionAsync(request with { State = "uncertain" }, CancellationToken.None).ConfigureAwait(false); throw;
                }
    ...
    public async Task<ResetHistoryPage> ResetHistoryAsync(string key, string? cursor, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var rows = root.At("events").Items().Select(item => new ResetHistoryEvent(item.At("id").Text() ?? throw HttpTransport.FormatError(),
                item.At("kind").Text() ?? throw HttpTransport.FormatError(), ReadDate(item.At("occurred_at")) ?? throw HttpTransport.FormatError())).ToArray();
            ...
            if (cursor is null) await database.PutMetadataAsync("reset_history_" + key, page, ct).ConfigureAwait(false); return page;
        }
        finally { gates[account.Provider].Release(); }
    }
