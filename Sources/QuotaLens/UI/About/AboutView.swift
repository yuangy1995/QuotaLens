        ...
        [
            ChangelogEntry(
                ...
                changes: [
                    ...
                    L10n.text("避免尚未写入完整身份的 Claude 子任务影响父会话层级", "Prevents Claude child tasks without a complete identity from changing the parent-session hierarchy"),
                    L10n.text("新增 Claude 本地记录完整度诊断与分块读取，损坏或格式变化会明确提示", "Adds Claude local-record completeness checks and chunked reading with clear warnings for damage or format changes"),
                    L10n.text("升级修复改为按来源重试，单个异常文件不再触发反复全量重建", "Retries upgrade repairs per source so one problematic file no longer triggers repeated full rebuilds"),
                    L10n.text("收紧账号别名迁移依据并去重重复扫描路径", "Tightens account-alias migration evidence and removes duplicate scan paths")
                ]
            ),
            ChangelogEntry(
                id: "v1.0.26",
